# TODO: Translation updated at 2023-10-14 21:25

translate french strings:

    # game/scripts/screens/laptop.rpy:21
    old "ОНЛАЙН-КУРСЫ"
    new "COURS EN LIGNE"

    # game/scripts/screens/laptop.rpy:21
    old "ИНТЕРНЕТ-МАГАЗИН"
    new "BOUTIQUE EN LIGNE"

    # game/scripts/screens/laptop.rpy:21
    old "ЗАНЯТЬСЯ СВОИМ САЙТОМ"
    new "POUR REPRENDRE LEUR SITE WEB"

    # game/scripts/screens/laptop.rpy:21
    old "СВОЙ САЙТ"
    new "VOTRE SITE"

    # game/scripts/screens/laptop.rpy:21
    old "CYBER-БАНК"
    new "CYBER-BANK"

    # game/scripts/screens/laptop.rpy:21
    old "ЧИТ-МЕНЮ"
    new "CHIT MENU"

    # game/scripts/screens/laptop.rpy:42
    old "Назад"
    new "Précédent"

    # game/scripts/screens/laptop.rpy:46
    old "Выключить"
    new "Éteindre"

    # game/scripts/screens/laptop.rpy:52
    old "Ждать 10 минут"
    new "Attendre 10 minutes"

    # game/scripts/screens/laptop.rpy:197
    old "КУПЛЕНО"
    new "ACHETÉ"

    # game/scripts/screens/laptop.rpy:202
    old "КУПИТЬ (недостаточно средств)"
    new "BUY (fonds insuffisants)"

    # game/scripts/screens/laptop.rpy:207
    old "КУПИТЬ"
    new "ACHETER"

    # game/scripts/screens/laptop.rpy:208
    old "Приобрести \"{name}\" за ${price}?"
    new "Achat \"{name}\" pour ${price}?"

    # game/scripts/screens/laptop.rpy:238
    old "Скидка [item.sale_percent]%"
    new "Remise [item.sale_percent]%"

    # game/scripts/screens/laptop.rpy:259
    old "В данный момент нет доступных товаров. Загляните позже."
    new "Il n'y a pas de produits disponibles pour le moment. Revenez plus tard."

    # game/scripts/screens/laptop.rpy:419
    old "Что будем искать?"
    new "Qu'est ce que je recherche?"

    # game/scripts/screens/laptop.rpy:470
    old "ВЫГОДНЫЕ ЗАЙМЫ ИНТЕРНЕТ-ПРЕДПРИНИМАТЕЛЯМ"
    new "PRÊTS INTERESSANT POUR LES ENTREPRENEURS D'INTERNET"

    # game/scripts/screens/laptop.rpy:474
    old "ВЗЯТЬ КРЕДИТ"
    new "CONTRACTER UN PRÊT"

    # game/scripts/screens/laptop.rpy:510
    old "Общий доход:"
    new "Revenu total:"

    # game/scripts/screens/laptop.rpy:511
    old "Последнее начисление:"
    new "Dernière régularisation :"

    # game/scripts/screens/laptop.rpy:588
    old "Общение"
    new "Réseaux Sociaux"

    # game/scripts/screens/laptop.rpy:603
    old "Вы прошли все доступные курсы и занятия из этой категории. Возможно, новые уроки появятся в следующей версии игры."
    new "Vous avez terminé tous les leçons et cours disponibles dans cette catégorie. Peut-être que de nouvelles leçons apparaîtront dans la prochaine version du jeu."

    # game/scripts/screens/laptop.rpy:611
    old "Вы уже оплатили этот курс и можете в любой момент включить следующий доступный видеоурок."
    new "Vous avez déjà payé pour ce cours et pouvez activer le prochain didacticiel vidéo disponible à tout moment."

    # game/scripts/screens/laptop.rpy:612
    old "Занятие [learn] из 3"
    new "Classe [learn] de 3"

    # game/scripts/screens/laptop.rpy:615
    old "НАЧАТЬ ПРОСМОТР ВИДЕОУРОКА"
    new "COMMENCER À REGARDER LA LEÇON VIDÉO"

    # game/scripts/screens/laptop.rpy:619
    old "ВЫ УЧИЛИСЬ СОВСЕМ НЕДАВНО. СДЕЛАЙТЕ ПЕРЕРЫВ!"
    new "QUE VOUS AVEZ ÉTUDIÉS RÉCEMMENT. FAITES UNE PAUSE !"

    # game/scripts/screens/laptop.rpy:627
    old "Количество занятий: 3"
    new "Nombre de classes : 3"

    # game/scripts/screens/laptop.rpy:629
    old "ПРИОБРЕСТИ ЭТОТ КУРС ЗА $[cource.price]"
    new "POUR ACHETER CE COURS AU PRIX DE[cource.price]"

