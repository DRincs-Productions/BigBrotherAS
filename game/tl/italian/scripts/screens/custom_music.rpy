# TODO: Translation updated at 2026-07-03 17:21

translate italian strings:

    # game/scripts/screens/custom_music.rpy:15
    old "Ok"
    new "Ok"

    # game/scripts/screens/custom_music.rpy:19
    old "Отмена"
    new "Annulla"

    # game/scripts/screens/custom_music.rpy:50
    old "Настройка музыки"
    new "Impostazioni musicali"

    # game/scripts/screens/custom_music.rpy:105
    old "Без музыки"
    new "Senza musica"

    # game/scripts/screens/custom_music.rpy:112
    old "В случайном порядке"
    new "In modo casuale"

    # game/scripts/screens/custom_music.rpy:119
    old "Сбросить"
    new "Reimposta"

    # game/scripts/screens/custom_music.rpy:169
    old "Пересканировать папку музыки"
    new "Effettuare una nuova scansione della cartella musica"

