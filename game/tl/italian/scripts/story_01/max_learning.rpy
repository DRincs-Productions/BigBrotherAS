# TODO: Translation updated at 2024-06-18 22:58

# game/scripts/story_01/max_learning.rpy:8
translate italian ev_029_47925c16:

    # Max 00 "Пришло время почитать..." nointeract
    Max 00 "È ora di leggere...[nointeract]"

# game/scripts/story_01/max_learning.rpy:14
translate italian ev_029_manual_fd324daa:

    # Max 13 "Хм... куча непонятных слов. Кажется, нужно будет заново перечитать первые главы..."
    Max 13 "Hm... Un sacco di parole incomprensibili. Credo che dovrò rileggere i primi capitoli..."

# game/scripts/story_01/max_learning.rpy:17
translate italian ev_029_manual_41775a3a:

    # Max 12 "Так, ну с этим я уже разобрался, хорошо... А это что такое? Не ясно. Нужно будет всё осмыслить..."
    Max 12 "Bene, ho risolto la questione, ok... Cos'è questo? Non è chiaro. Dovrò capirlo..."

# game/scripts/story_01/max_learning.rpy:20
translate italian ev_029_manual_b8327ffa:

    # Max 11 "Ого, вот это здорово! Уже можно делать сайт? А, нет... Ещё не всё понятно... Ну, разберусь в другой раз."
    Max 11 "Wow, è fantastico! È possibile creare un sito web? Ah, no... Non lo so ancora... Beh, ci penserò un'altra volta."

# game/scripts/story_01/max_learning.rpy:23
translate italian ev_029_manual_90d4232e:

    # Max 12 "Так, ну теперь картина вырисовывается. Осталось разобраться только с мелочами... Или это не мелочи?"
    Max 12 "Bene, il quadro si sta delineando. Restano solo le piccole cose da sistemare... O non sono le piccole cose?"

# game/scripts/story_01/max_learning.rpy:26
translate italian ev_029_manual_7b71f292:

    # Max 01 "Всё, вот теперь точно всё понятно! Я уже могу сделать свой сайт и транслировать на него изображение! Но как получать за это деньги? А, нет, это тоже понятно!"
    Max 01 "Ecco, ora tutto ha perfettamente senso! Posso già creare il mio sito web e trasmettervi un'immagine! Ma come faccio a guadagnare? Oh, no, anche questo ha senso!"

# game/scripts/story_01/max_learning.rpy:47
translate italian ev_046_23ca8fd9:

    # Max 01 "Хм... мне кажется такое можно и лёжа на кровати изучать. Надеюсь, меня не станет клонить в сон..."
    Max 01 "Ehm... Penso che si possa fare stando a letto. Spero di non addormentarmi..."

# game/scripts/story_01/max_learning.rpy:51
translate italian ev_046_66ef962f:

    # Max 11 "Хорошая штука эти онлайн-курсы. Можно научиться всему, не выходя из дома! Вот только и стоит это немало..."
    Max 11 "I corsi online sono un'ottima cosa. Si può imparare tutto comodamente da casa propria! Ma costa anche molto..."

# game/scripts/story_01/max_learning.rpy:55
translate italian ev_046_81dac6d3:

    # Max 04 "Не сказать, что это сложное занятие, но знаний, а так же опыта в этом деле нужно много! И теперь я готов закреплять полученные знания на практике..."
    Max 04 "Non posso dire che sia difficile, ma ci vuole molta conoscenza ed esperienza! E ora sono pronto a mettere in pratica ciò che ho imparato..."

# game/scripts/story_01/max_learning.rpy:62
translate italian ev_046_1c72ad2c:

    # tMax 01 "И раз уж с Алисы это началось, то на ней я и начну практиковаться. Надеюсь, руки она мне не выдернет, если я сделаю что-то не так..."
    tMax 01 "E visto che è iniziato con Alice, inizierò a esercitarmi su di lei. Spero che non mi strappi le braccia se faccio qualcosa di sbagliato..."

# game/scripts/story_01/max_learning.rpy:73
translate italian ev_046_1ec95453:

    # tMax 01 "Мне как раз нужно убедить Алису в том, что показывать стриптиз через вэб-камеру - не так уж и плохо. И даже может быть прибыльно, если она будет это делать с чувством..."
    tMax 01 "Devo proprio convincere Alice che fare uno spogliarello attraverso la webcam non è poi così male. E potrebbe anche essere redditizio, se lo farà con passione..."

# game/scripts/story_01/max_learning.rpy:76
translate italian ev_046_66ef962f_1:

    # Max 11 "Хорошая штука эти онлайн-курсы. Можно научиться всему, не выходя из дома! Вот только и стоит это немало..."
    Max 11 "I corsi online sono un'ottima cosa. Si può imparare tutto comodamente da casa propria! Ma costa anche molto..."

translate italian strings:

    # game/scripts/story_01/max_learning.rpy:7
    old "{i}читать «Веб-стандарты»{/i}"
    new "{i}leggi «Standard web»{/i}"

    # game/scripts/story_01/max_learning.rpy:15
    old "Книга изучена на 20%"
    new "Il libro è stato studiato al 20%"

    # game/scripts/story_01/max_learning.rpy:18
    old "Книга изучена на 40%"
    new "Il libro è stato studiato al 40%"

    # game/scripts/story_01/max_learning.rpy:21
    old "Книга изучена на 60%"
    new "Il libro è stato studiato al 60%"

    # game/scripts/story_01/max_learning.rpy:24
    old "Книга изучена на 80%"
    new "Il libro è stato studiato all'80%"

    # game/scripts/story_01/max_learning.rpy:27
    old "Книга полностью изучена"
    new "Il libro è stato studiato in modo approfondito"

    # game/scripts/story_01/max_learning.rpy:48
    old "Курс изучен на 33%"
    new "Il corso è completo al 33%"

    # game/scripts/story_01/max_learning.rpy:52
    old "Курс изучен на 66%"
    new "Il corso è completo al 66%"

    # game/scripts/story_01/max_learning.rpy:58
    old "Курс полностью изучен"
    new "Il corso è stato completamente approfondito"

    # game/scripts/story_01/max_learning.rpy:64
    old "Получен навык «Массаж»!"
    new "L'abilità «Massaggio» è stata ottenuta!"

    # game/scripts/story_01/max_learning.rpy:65
    old "Задание «Домашнее обучение массажу» завершено!"
    new "La missione «Formazione sul massaggio a domicilio» è stata completata!"

    # game/scripts/story_01/max_learning.rpy:66
    old "Открыто новое задание «Талантливый массажист»!"
    new "Una nuova missione «Massaggiatore di talento» è aperta!"

