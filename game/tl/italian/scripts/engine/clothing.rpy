# TODO: Translation updated at 2024-12-25 17:41

translate italian strings:

    # game/scripts/engine/clothing.rpy:787
    old "Галерея переодеваний обновлена!"
    new "La galleria dei travestiti è stata aggiornata!"

    # game/scripts/engine/clothing.rpy:923
    old "Лонгслив + джинсы"
    new "Manica lunga + jeans"

    # game/scripts/engine/clothing.rpy:923
    old "Шорты"
    new "Pantaloncini"

    # game/scripts/engine/clothing.rpy:966
    old "Обнажённая"
    new "Nuda"

    # game/scripts/engine/clothing.rpy:966
    old "Джинсы"
    new "Jeans"

    # game/scripts/engine/clothing.rpy:966
    old "Съехавшая майка + джинсы"
    new "Maglietta + jeans"

    # game/scripts/engine/clothing.rpy:966
    old "Майка + трусики"
    new "Maglietta + mutande"

    # game/scripts/engine/clothing.rpy:966
    old "Майка + съехавшие трусики"
    new "Maglietta + pantaloni larghi"

    # game/scripts/engine/clothing.rpy:966
    old "Съехавшая майка + трусики"
    new "Maglietta + mutandine"

    # game/scripts/engine/clothing.rpy:966
    old "Съехавшие майка и трусики"
    new "La maglietta e le mutande si tolgono"

    # game/scripts/engine/clothing.rpy:966
    old "Трусики"
    new "Mutandine"

    # game/scripts/engine/clothing.rpy:966
    old "Съехавшие трусики"
    new "Mutandine che cadono"

    # game/scripts/engine/clothing.rpy:966
    old "Майка"
    new "Canottiera"

    # game/scripts/engine/clothing.rpy:966
    old "Съехавшая майка"
    new "La canottiera è tolta"

    # game/scripts/engine/clothing.rpy:966
    old "Приподнятое выходное платье + трусики"
    new "Abito da weekend rialzato + mutande"

    # game/scripts/engine/clothing.rpy:966
    old "Приподнятое и съехавшее выходное платье + трусики"
    new "Vestito da weekend sollevato e tirato giù + mutandine"

    # game/scripts/engine/clothing.rpy:966
    old "Приподнятое выходное платье"
    new "Vestito per il fine settimana di alto livello"

    # game/scripts/engine/clothing.rpy:966
    old "Приподнятое и съехавшее выходное платье"
    new "Vestito da weekend sollevato e tirato giù"

    # game/scripts/engine/clothing.rpy:966
    old "Нижнее бельё, съехавшее снизу (синее)"
    new "La biancheria intima è scivolata in basso (blu)"

    # game/scripts/engine/clothing.rpy:966
    old "Нижнее бельё, съехавшее сверху (синее)"
    new "La biancheria intima, sfilata dalla parte superiore (blu)"

    # game/scripts/engine/clothing.rpy:966
    old "Нижнее бельё, съехавшее сверху и снизу (синее)"
    new "La biancheria intima è scivolata in basso (blu)"

    # game/scripts/engine/clothing.rpy:966
    old "Трусики от нижнего белья (синие)"
    new "Mutandine intimo (blu)"

    # game/scripts/engine/clothing.rpy:966
    old "Съехавшие трусики от нижнего белья (синие)"
    new "Mutandine che si sfilano (blu)"

    # game/scripts/engine/clothing.rpy:966
    old "Бикини, съехавшее сверху (чёрное)"
    new "Bikini con top-slip (nero)"

    # game/scripts/engine/clothing.rpy:966
    old "Трусики от бикини (чёрные)"
    new "Mutandine bikini (nero)"

    # game/scripts/engine/clothing.rpy:966
    old "Распахнутый халат"
    new "Vestaglia aperta"

    # game/scripts/engine/clothing.rpy:966
    old "Распахнутый халат + трусики"
    new "Vestaglia aperta + mutandine"

    # game/scripts/engine/clothing.rpy:966
    old "Нижнее бельё (чёрное)"
    new "Biancheria intima (nera)"

    # game/scripts/engine/clothing.rpy:966
    old "Нижнее бельё (чёрное), съехавшее снизу"
    new "Biancheria intima (nera) spostata in basso"

    # game/scripts/engine/clothing.rpy:966
    old "Нижнее бельё, съехавшее сверху (чёрное)"
    new "Biancheria intima, reggiseno abbassato da un lato (nero)"

    # game/scripts/engine/clothing.rpy:966
    old "Нижнее бельё, съехавшее сверху и снизу (чёрное)"
    new "Biancheria intima spostata dall'alto verso il basso (nero)"

    # game/scripts/engine/clothing.rpy:966
    old "Трусики от нижнего белья (чёрные)"
    new "Mutandine intime (nere)"

    # game/scripts/engine/clothing.rpy:966
    old "Съехавшие трусики от нижнего белья (чёрные)"
    new "Mutandine che si sfilano (nere)"

    # game/scripts/engine/clothing.rpy:966
    old "Бикини, съехавшее сверху (светлое)"
    new "Bikini, top-slip (leggero)"

    # game/scripts/engine/clothing.rpy:966
    old "Трусики от бикини (светлые)"
    new "Mutandine bikini (colore chiaro)"

    # game/scripts/engine/clothing.rpy:966
    old "Съехавший халат"
    new "Accappatoio ampio"

    # game/scripts/engine/clothing.rpy:966
    old "Съехавшая ночнушка"
    new "Camicia da notte larga"

    # game/scripts/engine/clothing.rpy:966
    old "Приподнятое и съехавшее вечернее платье"
    new "Un abito da sera sollevato e scivolato"

    # game/scripts/engine/clothing.rpy:966
    old "Джинсовые шорты + нижнее бельё (светлое)"
    new "Pantaloncini di jeans + biancheria intima (colore chiaro)"

    # game/scripts/engine/clothing.rpy:966
    old "Джинсовые шорты + нижнее бельё, съехавшее сверху (светлое)"
    new "Pantaloncini di jeans + biancheria intima con la parte superiore abbassata (colore chiaro)"

    # game/scripts/engine/clothing.rpy:966
    old "Школьная юбка + нижнее бельё (светлое)"
    new "Gonna scolastica + biancheria intima (leggera)"

    # game/scripts/engine/clothing.rpy:966
    old "Школьная юбка + нижнее бельё, съехавшее сверху (светлое)"
    new "Gonna scolastica + biancheria intima con il top abbassato (colore chiaro)"

    # game/scripts/engine/clothing.rpy:966
    old "Нижнее бельё (светлое)"
    new "Biancheria intima (chiara)"

    # game/scripts/engine/clothing.rpy:966
    old "Нижнее бельё, съехавшее снизу (светлое)"
    new "Biancheria intima spostata dall'alto verso il basso (nero)"

    # game/scripts/engine/clothing.rpy:966
    old "Нижнее бельё, съехавшее сверху (светлое)"
    new "Biancheria intima tirata giù sopra la parte superiore (di colore chiaro)"

    # game/scripts/engine/clothing.rpy:966
    old "Нижнее бельё, съехавшее сверху и снизу (светлое)"
    new "Biancheria intima che si è staccata nella parte superiore e inferiore (di colore chiaro)"

    # game/scripts/engine/clothing.rpy:966
    old "Трусики от нижнего белья (светлые)"
    new "Mutandine intime (chiare)"

    # game/scripts/engine/clothing.rpy:966
    old "Съехавшие трусики от нижнего белья (светлые)"
    new "Mutandine da biancheria intima scivolate giù (chiare)"

    # game/scripts/engine/clothing.rpy:966
    old "Съехавшее выходное платье"
    new "Abito per il weekend"

    # game/scripts/engine/clothing.rpy:966
    old "Пижама, съехавшая сверху"
    new "Pigiama che scivolava giù dall'alto"

    # game/scripts/engine/clothing.rpy:966
    old "Пижамные шорты"
    new "Pantaloncini del pigiama"

    # game/scripts/engine/clothing.rpy:966
    old "Съехавший закрытый купальник"
    new "Costume da bagno intero"

    # game/scripts/engine/clothing.rpy:1122
    old "Повседневная"
    new "Informale"

